    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-600 mt-1">Track your financial health and progress</p>
        </div>
        ...
        <CardContent className="p-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h2 className="text-2xl font-bold">Level 12 Financial Hero</h2>
                <div className="flex items-center space-x-2 mt-1">
                  <Star className="h-4 w-4" />
                  <span className="text-lg">15,680 points</span>
                </div>
              </div>
            </div>
            <div className="text-right">
              <p className="text-sm opacity-90 mb-2">XP Progress</p>
              <p className="text-xl font-bold mb-2">2450 / 3000</p>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-gray-600">Detailed breakdown of your expenses with trends</p>
        </CardHeader>
        <CardContent>
          <div className="space-y-6">
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{category.percentage}% of total spending</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-bold">${category.total}</div>
                    <div
                      className={`text-sm flex items-center ${category.trendUp === true ? "text-red-600" : category.trendUp === false ? "text-green-600" : "text-gray-600"}`}
                    >
                      {category.trendUp === true && <TrendingUp className="h-3 w-3 mr-1" />}
                      {category.trendUp === false && <TrendingDown className="h-3 w-3 mr-1" />}
                      {category.trend}
                    </div>
                  </div>
                </div>
                  ...
                  {category.subcategories.map((sub, subIndex) => (
                    <div key={subIndex} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <span className="text-sm font-medium w-12 text-right">${sub.amount}</span>
                        <span className="text-xs text-gray-500 w-8">{sub.percentage}%</span>
                      </div>
                    </div>
                  ))}
                </div>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-sm text-gray-600">How your spending in each category changes over time</p>
        </CardHeader>
        <CardContent>
          <div className="h-[400px] relative">
            <svg className="w-full h-full" viewBox="0 0 700 350">
              ...
              {/* Y-axis labels */}
              {[0, 200, 400, 600, 800, 1000].map((y) => (
                <text key={y} x="50" y={305 - (y / 1000) * 240} fontSize="12" fill="#6b7280" textAnchor="end">
                  ${y}
                </text>
              ))}

              {/* X-axis labels */}
              {monthlySpendingByCategory.map((data, index) => (
                <text key={data.month} x={100 + index * 100} y="325" fontSize="12" fill="#6b7280" textAnchor="middle">
                  {data.month}
                </text>
              ))}

              {/* Category lines */}
              {Object.entries(categorySpending).map(([_, category], categoryIndex) => {
                const categoryKey =
                  category.category.toLowerCase().split(" ")[0] === "food"
                    ? "food"
                    : category.category.toLowerCase().includes("transport")
                      ? "transport"
                      : category.category.toLowerCase().includes("entertainment")
                        ? "entertainment"
                        : category.category.toLowerCase().includes("shopping")
                          ? "shopping"
                          : category.category.toLowerCase().includes("healthcare")
                            ? "healthcare"
                            : "utilities"
                ...
                return (
                  <g key={categoryIndex}>
                    <path
                      d={`M ${monthlySpendingByCategory
                        .map((data, index) => `${100 + index * 100},${300 - (data[categoryKey] / 1000) * 240}`)
                        .join(" L ")}`}
                      fill="none"
                    ...
                    {monthlySpendingByCategory.map((data, index) => (
                      <circle
                        key={index}
                        cx={100 + index * 100}
                        cy={300 - (data[categoryKey] / 1000) * 240}
                        r="3"
                        fill={category.color}
                        stroke="white"
                        strokeWidth="1"
              ...
              <g transform="translate(100, 20)">
                {categorySpending.slice(0, 3).map((category, index) => (
                  <g key={index} transform={`translate(${index * 120}, 0)`}>
                    <circle cx="6" cy="6" r="4" fill={category.color} />
                    <text x="15" y="10" fontSize="11" fill="#6b7280">
              ...
              <g transform="translate(100, 40)">
                {categorySpending.slice(3, 6).map((category, index) => (
                  <g key={index} transform={`translate(${index * 120}, 0)`}>
                    <circle cx="6" cy="6" r="4" fill={category.color} />
                    <text x="15" y="10" fontSize="11" fill="#6b7280">
                      {category.category.split(" ")[0]}
